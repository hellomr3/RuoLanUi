...
fun WeTimePicker(
    ...
    WePicker(
        ...
        val date = LocalTime(
            hour = rangesSource.first()[it.first()],
            minute = rangesSource.getOrNull(1)?.get(it[1]) ?: 0,
            second = rangesSource.getOrNull(2)?.get(it[2]) ?: 0
        )
        onChange(date)
    }
}
...
private fun rememberRangesSource(
    ...
    println("start:${start.hour},end:${end.hour}")
...
interface TimePickerState {
    ...
    fun show(
        value: LocalTime? = null,
        type: TimeType = TimeType.SECOND,
        start: LocalTime = LocalTime.fromSecondOfDay(0),
        end: LocalTime = LocalTime.fromSecondOfDay(86399),
        onChange: (LocalTime) -> Unit
    )
